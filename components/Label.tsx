...
export function Label({ children, variant, small, className }: Props) {
	return (
		<div
			className={clsx(
				'text-xs rounded-md leading-5 flex items-center',
				{
					'bg-brand-500 text-brand-contrast': variant === 'brand',
					'bg-primary text-primary p-1': variant === 'default',
					'border border-accent': variant === 'border',
					'text-brand-700 bg-brand-100 dark:text-black dark:bg-brand-500':
						variant === 'cool',
